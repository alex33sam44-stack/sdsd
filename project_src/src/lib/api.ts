...
type RequestOptions = {
  /** Send/receive cookies for cross-origin requests. Used by the OAuth handoff. */
...
async function request<T>(method: Method, path: string, body?: unknown, retried = false, options?: RequestOptions): Promise<T> {
  if (!BASE_URL) {
  ...
  const res = await fetch(`${BASE_URL}${path}`, {
    ...
    // Defaults to "same-origin"; opted in only for endpoints that need to
    // read short-lived auth handoff cookies (e.g. /auth/exchange) so we don't
    // expand cookie surface for every API call.
    credentials: options?.withCredentials ? "include" : "same-origin",
  });

  if (res.status === 401 && !retried) {
    const refreshed = await refreshOnce();
    if (refreshed) return request<T>(method, path, body, true, options);
    tokenStore.clear();
  }
...
export const authApi = {
  ...
  /**
   * Pulls tokens from the short-lived HttpOnly handoff cookies set by the
   * Google OAuth callback. Tokens are NEVER placed in the URL — the redirect
   * carries only a `?oauth_exchange=1` flag, and the cookies are cleared
   * server-side as soon as we read them, so an interrupted handoff cannot
   * be replayed.
   */
