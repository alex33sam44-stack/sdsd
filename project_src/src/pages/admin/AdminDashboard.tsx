...
import { BarChart3, Building2, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, CreditCard, FileEdit, FileWarning, Globe2, History, Lightbulb, Map, Plus, ShieldCheck, Trash2, Users } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-5 pt-5 pb-10 space-y-5">
        ...
        {/* ─── Primary tools: always visible, daily operator essentials ─── */}
        ...
        {/* ─── Advanced tools: collapsed by default to reduce cognitive load ─── */}
        <AdvancedSection
          stationId={stationId}
          isAdmin={isAdmin}
...
// Keeps the dashboard lean for simple operators; power-users can expand when needed.
const AdvancedSection = ({ stationId, isAdmin, currentTenant, ent, Chevron, t }: {
  stationId: string;
  isAdmin: boolean;
  currentTenant: any;
  ent: any;
  ...
  return (
    <section className="space-y-3">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between rounded-xl border-2 border-dashed border-secondary/40 bg-surface p-4 shadow-tactile-sm active:translate-y-0.5 active:shadow-none transition-transform"
      >
        <span className="font-black text-secondary text-sm">
          {open ? t("admin.hideAdvanced", "إخفاء الأدوات المتقدمة") : t("admin.showAdvanced", "أدوات متقدمة ٧+")}
        </span>
        <ToggleIcon className="w-5 h-5 text-secondary" strokeWidth={2.5} />
      </button>
