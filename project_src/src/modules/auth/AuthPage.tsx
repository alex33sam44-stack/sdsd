...
const AuthPage = () => {
  ...
  useEffect(() => {
    ...
    // OAuth handoff: the backend used to put access_token + refresh_token in
    // the URL fragment, which leaked them to browser history and any
    // analytics that captured location.href. We now redirect with a single
    // non-secret `?oauth_exchange=1` flag and stash the actual tokens in
    // short-lived HttpOnly cookies. Calling /auth/exchange returns the
    // tokens once and clears the cookies server-side.
    const oauthExchange = url.searchParams.get("oauth_exchange");
    if (oauthExchange === "1") {
      // Strip the flag immediately so a refresh can't re-trigger the call.
      url.searchParams.delete("oauth_exchange");
      ...
      authApi
        ...
        .catch(() => {
          // Stay on the auth page; the user can retry sign-in.
